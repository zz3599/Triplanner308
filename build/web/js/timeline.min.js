var Timeline = function(container, date) {
    ...
    this.addEvent = function(event) {
        ...
        var newevent = {
            ...
            content: event.comment + '<br>Location: ' + event.startLocation + (!event.endLocation ? '' : ('to ' + event.endLocation)),
            eventid: event.id,
    ...
    this.renderDay = function(topRow, bottomRow, data) {
        ...
        cssClass += (data.events.length || this.isDateInRange(data)) ? " event" : "";
    ...
    this.showDayDetails = function(event) {
        ...
        if (infobar.css('display') !== 'none') {
            ...
            $.get(window.app.TRIPDAY, {'date': this.date.getTime(), 'tripid': self.tripid}).success(function(tripday) {
                ...
                $.get(window.app.WAYPOINT, {'tripid': self.tripid, 'tripdayid': self.tripdayid}).success(function(d) {
                    ...
                    $.each(data, function(i, waypoint) {
                        var elem = $(Mustache.render(self.WAYPOINTTEMPLATE, waypoint)).appendTo(parent);
                    });
                });
            });
        }
...
Timeline.Bubble = function(timeline) {
    ...
    this.setContent = function(events) {
        ...
        for (var i = 0; i < events.length; i++)
            html += "<div><div title='" + events[i].name + "' tripdayid='" + events[i].tripdayid + "' eventid='" + events[i].eventid + "' class=\"event_title\">" + events[i].name + "<p class=\"event_data\">" + events[i].content + "</p></div></div>";
        ...
        titles.each(function(idx, element) {
            ...
            $(element).click(function() {
                $('#eventdescription').val($(this).attr('title'));
                $('#photoeventid').val($(this).attr('eventid'));
                $('#phototripdayid').val($(this).attr('tripdayid'));
            });
        });
        var addEvent = midSection.find('#addevent');
        addEvent.click(function() {
            if (!$('#eventtripid').val() || !$('#eventtripdayid').val()) {
                alert('Please select a day first');
                return;
            }
            ;
            $("#dialog-form").dialog("open");
            var minDate = new Date($('#date').val());
            var maxDate = new Date(minDate.getTime());
            maxDate.setDate(minDate.getDate() + 1);
            var eventstart = $('#eventStart').removeClass('hasDatepicker');
            var eventend = $('#eventEnd').removeClass('hasDatepicker');
            window.app.initTimepickers(eventstart, eventend, minDate, maxDate);
        });
    };
    this.initAddEventModal = function() {
        var self = this;
        ...
        $("#dialog-form").dialog({
            ...
            buttons: {
                "Finish": function() {
                    var bValid = true;
                    bValid = bValid && valid(description.val()) && valid(startLocation.val()) &&
                            valid(startTime.val()) && valid(endTime.val());
                    if (bValid) {
                        //add to the timeline
                        //add to the db
                        window.app.initSpinner();
                        $.post('events', $('#neweventform').serialize()).success(function(data) {
                            window.app.stopSpinner();
                            window.app.timeline.addEvent(JSON.parse(data));
                            window.app.timeline.render();
                        });
                        $(this).dialog("close");
                    }
                },
